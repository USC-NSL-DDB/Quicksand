...
std::atomic<int> num_cps;
vector<std::pair<int, int>> cp_logs;
...
void log_cp_begin() {
  ...
  int time = microtime();
  auto num = num_cps++;
  cp_logs.emplace_back(time, num);
  cp_logs.emplace_back(time + 1, num + 1);
}
...
struct CPLogger {
  CPLogger() { log_cp_begin(); }

  ~CPLogger() { log_cp_end(); }
};

tuple<vector<RasterHandle>, uint16_t, uint16_t> read_raster(
    const BufferReader &buffer_reader) {
  ...
  tuple<vector<RasterHandle>, uint16_t, uint16_t> output(
      original_rasters, input_reader.display_width(),
...
tuple<IVF_MEM, DecoderBuffer> enc_given_state(IVF_MEM &pred_ivf,
                                              DecoderBuffer &input_state,
                                              DecoderBuffer *prev_state,
                                              size_t idx, const string prefix,
  ...
  if (not pred_decoder.minihash_match(pred_ivf.expected_decoder_minihash())) {
    throw Invalid(
        "Mismatch between prediction IVF and prediction_ivf_initial_state");
  }

  for (unsigned int i = 0; i < pred_ivf.frame_count(); i++) {
    UncompressedChunk unch{pred_ivf.frame(i), pred_ivf.width(),
                           pred_ivf.height(), false};
  ...
  Encoder encoder(EncoderStateDeserializer_MEM::build<Decoder>(input_state),
                  two_pass, quality);

  ivf_writer.set_expected_decoder_entry_hash(
      encoder.export_decoder().get_hash().hash());
...
void decode_all(shared_ptr<xc_t> s) {
  ...
  auto vecs = vector<tuple<DecoderBuffer, task_time>>(join_view.begin(),
                                                      join_view.end());
...
void encode_all(shared_ptr<xc_t> s, const string prefix) {
  ...
  auto encode_range = nu::make_zipped_ds_range(
      sealed_ivfs, sealed_dec_state, sealed_raster_buffer, sealed_idxs);
  ...
  auto dist_exec = nu::make_distributed_executor(
      +[](decltype(encode_range) &encode_range, const string prefix) {
        ...
        while (true) {
          ...
          auto out = enc_given_state(ivf, decoder, nullptr, idx, prefix,
                                     raster_buffer);
          out = enc_given_state(ivf, decoder, nullptr, idx, prefix,
                                raster_buffer);
  ...
  auto join_view = ranges::join_view(outputs_vectors);
  auto vecs = vector<tuple<IVF_MEM, DecoderBuffer, task_time>>(
      join_view.begin(), join_view.end());
...
void rebase(shared_ptr<xc_t> s, const string prefix) {
  ...
  for (size_t i = 1;
       ivf_it != sealed_ivfs.cend() && state_it != sealed_dec_state.cend() &&
       buffer_it != sealed_raster_buffer.cend();
       ++ivf_it, ++state_it, ++buffer_it, ++i) {
    task_time t;
    t.start = microtime();
    IVF_MEM ivf = *ivf_it;
    ...
    auto output =
        enc_given_state(ivf, decoder, &prev_decoder, i, prefix, buffer);
...
void do_work(int argc, char **argv) {
  ...
  cout << "cp log: " << endl;
  for (auto &[_, cp] : cp_logs) {
    cp = std::max(cp, 1);
  }
  cp_logs.front().second = cp_logs.back().second = 0;
  for (auto &[us, cp] : cp_logs) {
    cout << us << " " << cp << endl;
  }

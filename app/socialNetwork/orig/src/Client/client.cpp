...
constexpr static double kTargetMops = 0.002;
constexpr static double kTotalMops = 1.0;
constexpr static char kLocalHost[] = "127.0.0.1";
...
public:
  std::unique_ptr<nu::PerfThreadState> create_thread_state() {
    ...
    state->user_timeline_client.init(std::string(kLocalHost),
                                     kUserTimeLinePort);
    state->home_timeline_client.init(std::string(kLocalHost),
                                     kHomeTimeLinePort);
    state->compose_post_client.init(std::string(kLocalHost), kComposePostPort);
    state->social_graph_client.init(std::string(kLocalHost), kSocialGraphPort);

      ...
      case 4:
        switch (mid - r) {
          case 0:
            goto select_min;
          case 1:
            partition4<false>(r, 0, 1, 2, 3);
            break;
          case 2:
            partition4<true>(r, 0, 1, 2, 3);
            break;
          case 3:
            goto select_max;
          default:
...
*/
template <class T>
size_t expandPartition(T* r, size_t lo, size_t pivot, size_t hi,
                       size_t length) {
  ...
  for (;; ++left, --length) {
    ...
    for (;; --length) {
      if (length == hi)
        return left + expandPartitionLeft(r + left, lo - left, pivot - left);

...
size_t medianOfMinima(T* const r, const size_t n, const size_t length) {
  ...
  for (size_t i = 0, j = subset; i < subset; ++i) {
    ...
    size_t minIndex = j;
    while (++j < limit)
      if (r[j] < CNT r[minIndex]) minIndex = j;
    if (r[minIndex] < CNT r[i]) cswap(r[i], r[minIndex]);
...
size_t medianOfMaxima(T* const r, const size_t n, const size_t length) {
  ...
  for (size_t i = subsetStart, j = i - subset * computeMaxOver; i < length;
    ...
    size_t maxIndex = j;
    while (++j < limit)
      if (r[j] > CNT r[maxIndex]) maxIndex = j;
    if (r[maxIndex] > CNT r[i]) cswap(r[i], r[maxIndex]);
...
size_t medianOfNinthers(T* const r, const size_t length) {
  assert(length >= 12);
  const auto frac = length <= 1024         ? length / 12

...
template <class Shard>
inline ssize_t ContiguousDSRangeImpl<Shard>::size() const {
  return static_cast<ssize_t>(rt::access_once(initial_key_range_.second)) -
         static_cast<ssize_t>(cur_key_);
}
...
inline ContiguousDSRangeImpl<Shard> ContiguousDSRangeImpl<Shard>::split() {
  ...
  return __split(mid_key);
}

template <class Shard>
ContiguousDSRangeImpl<Shard> ContiguousDSRangeImpl<Shard>::__split(
    std::size_t mid_key) {
  auto end_key = initial_key_range_.second;
  initial_key_range_.second = mid_key;
  mb();
  if (likely(cur_key_ < mid_key)) {

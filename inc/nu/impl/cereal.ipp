...
consteval bool is_memcpy_safe() {
  if constexpr (std::is_trivially_copy_assignable_v<T> &&
    ...
    return []<std::size_t... ints>(
               std::integer_sequence<std::size_t, ints...> int_seq) {
...
template <class Archive, typename T,
          traits::EnableIf<cereal::traits::is_same_archive<
              Archive, cereal::BinaryOutputArchive>::value>>
inline void save_move(Archive &ar, T &&t)
  requires(is_memcpy_safe<T>() && !HasBuiltinSerialize<Archive, T> &&
           !HasBuiltinSave<Archive, T> && !HasBuiltinLoad<Archive, T> &&
           !cereal::common_detail::is_enum<T>::value &&
           !nu::is_specialization_of_v<T, cereal::BinaryData> &&
...
inline void save(Archive &ar, std::vector<P, A> const &v)
...
template <class Archive, typename P, typename A>
inline void save_move(Archive &ar, std::vector<P, A> &&v)
  requires(is_memcpy_safe<P>())
{
  ar(v.size());
  ar(cereal::binary_data(v.data(), v.size() * sizeof(P)));
...
template <class Archive, typename P, typename A>
inline void load(Archive &ar, std::vector<P, A> &v)
  requires(is_memcpy_safe<P>())
{
...
void SizeArchive::operator()(const Ts &...ts)

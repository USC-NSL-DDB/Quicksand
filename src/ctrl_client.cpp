...
ControllerClient::ControllerClient(NodeIP ctrl_server_ip, lpid_t lpid,
                                   bool main, bool isol)
    : lpid_(lpid),
  ...
  auto optional = register_node(get_cfg_ip(), md5, main, isol);
  BUG_ON(!optional);
...
std::optional<std::pair<lpid_t, VAddrRange>> ControllerClient::register_node(
    NodeIP ip, MD5Val md5, bool main, bool isol) {

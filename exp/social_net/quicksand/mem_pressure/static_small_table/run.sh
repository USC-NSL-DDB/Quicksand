...
cd $NU_DIR
cp inc/nu/sharded_ds.hpp inc/nu/sharded_ds.hpp.bak
sed "s/\(.*\)kLowLatencyMaxShardBytes =.*/\1kLowLatencyMaxShardBytes = 200 << 20;/g" \
    -i inc/nu/sharded_ds.hpp
make clean
make -j30

cd $SOCIAL_NET_DIR
cp src/states.cpp src/states.cpp.bak
cp $DIR/states.cpp src/states.cpp
...
cd $NU_DIR
mv inc/nu/sharded_ds.hpp.bak inc/nu/sharded_ds.hpp
make clean
make -j30

cd $SOCIAL_NET_DIR
mv src/states.cpp.bak src/states.cpp
cd build
make clean
make -j
